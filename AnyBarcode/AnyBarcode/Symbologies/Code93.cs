namespace AnyBarcode.Symbologies
{
    ...
    public class Code93 : BarcodeSymbology
    {
        ...
        private string AddCheckDigits(string input)
        {
            ...
            {
                sum += aryCWeights[i] * int.Parse(_c93Code.Select("Character = '" + input[i] + "'")[0]["Value"]?.ToString() ?? string.Empty);
            }
            ...
            for (var i = 0; i < input.Length; i++)
            {
                sum += aryKWeights[i] * int.Parse(_c93Code.Select("Character = '" + input[i] + "'")[0]["Value"].ToString() ?? string.Empty);
            }
